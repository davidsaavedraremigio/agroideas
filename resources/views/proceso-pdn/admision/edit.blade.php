        ...
        <div class="card card-primary card-outline card-outline-tabs">
            <div class="card-header p-0 pt-1 border-bottom-0">
                <ul class="nav nav-tabs" id="TabExpediente" role="tablist">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" id="TabExpediente002" data-toggle="pill" href="#custom-tabs-proceso" role="tab" aria-controls="custom-tabs-proceso" aria-selected="false">2. Etapas de la admisión</a>
                    </li>
                </ul>
                <div class="card-body">
                    <div class="tab-content" id="TabExpedienteContent">
                        <div class="tab-pane fade show active" id="custom-tabs-expediente" role="tabpanel" aria-labelledby="TabExpediente001">
                            {!!Form::model($expediente,['id'=>'FormUpdateExpediente', 'method'=>'POST', 'files' => 'true', 'enctype' => 'multipart/form-data', 'route'=>['admision.update',$expediente->id]])!!}
                            {{Form::token()}}
                            ...
                            <div class="form-group">
                                <div class="row">
                                    ...
                                    <div class="col-md-3">{!! Form::label('personal', 'Especialista asignado') !!}
                                    ...
                                    <div class="col-md-3">{!! Form::label('estado', 'Situación del expediente') !!}
                                        <select name="estado" class="form-control select2">
                                            <option value="" selected="selected">Seleccionar</option>
                                            @foreach ($situacion as $fila)
                                ...
                                <div class="row">
                                    <div class="col-xs-12">
                                        <div id="Footer_UpdateExpediente_Enabled">
                                           ...
                                           <a href="{{ env('APP_URL') }}/proceso-pdn/admision" class="btn btn-sm btn-default"><i class="fas fa-sign-out-alt"></i> Cerrar formulario</a>
                                       </div>
                        ...
                        <div class="tab-pane fade" id="custom-tabs-proceso" role="tabpanel" aria-labelledby="TabExpediente002">
                            {{-- Contenido del módulo Procesos --}}
                            <section class="content">
                                <div class="container-fluid">
                                    <div class="card card-default color-palette-box">
                                        <div class="card-header">
                                            <h3 class="card-title">Registro de procesos correspondientes a la admisión del expediente</h3>
                                            <div class="card-tools">
                                                <a href="#" class="btn btn-sm btn-primary" data-toggle="modal" data-target="#modalCreateProcesoExpediente"><i class="fa fa-plus" aria-hidden="true"></i><span> Añadir nuevo</span></a>
                                            </div>
                                        </div>
                                        <div class="card-body">
                                            <div id="viewDataProcesoExpediente" class="table-responsive" data-id="{{$expediente->id}}"></div>
                                        </div>
                                    </div>
                                </div>
                            </section>
...
<div class="modal fade" id="modalCreateProcesoExpediente">
    <div class="modal-dialog modal-lg">
        <div class="modal-content animated fadeIn ">
            {{-- Inicio del contenido del modal --}}
            <div id="divFormCreateProcesoExpediente">
...
<div class="modal fade" id="modalUpdateProcesoExpediente">
    <div class="modal-dialog modal-lg">
        <div class="modal-content animated fadeIn ">
            {{-- Inicio del contenido del modal --}}
            <div id="divFormUpdateProcesoExpediente">
                <i class="fa fa-refresh fa-spin fa-2x fa-fw"></i>
                <span class="sr-only">Cargando...</span>
            </div>
            {{-- Fin del contenido del modal --}}
...
<script>
    $(document).ready(function () {
        ...
        //2. Cargo la info correspondiente a los procesos
        var codExpediente    =   $("#viewDataProcesoExpediente").attr('data-id');
        $("#viewDataProcesoExpediente").html("<i class='fa fa-spinner fa-pulse fa-2x fa-fw'></i><span class='sr-only'>Cargando...</span><h5>Espere un momento por favor, obteniendo datos ...</h5>");
        $("#viewDataProcesoExpediente").load(urlApp+'/proceso-pdn/proceso/'+ codExpediente +'/data');
        //3. Muestro los modals correspondientes a los procesos
        $('#modalCreateProcesoExpediente').on('show.bs.modal', function (e) {
            $('#divFormCreateProcesoExpediente').load(urlApp+'/proceso-pdn/proceso/' + codExpediente + '/create');
        });
        $('#modalUpdateProcesoExpediente').on('show.bs.modal', function (e) {
            var codProceso= $(e.relatedTarget).attr('data-id');
            $('#divFormUpdateProcesoExpediente').load(urlApp+'/proceso-pdn/proceso/'+codProceso+'/edit');
        });
        //4. Proceso los formularios de procesos
        $(document).on("click", '#btnCreateProcesoExpediente', function (event) {
            event.preventDefault();
            var form = $("#FormCreateProcesoExpediente");
            var urlAction = form.attr('action');
            var formData = new FormData(form[0]);
            var dataAll = form.serialize();
            $.ajax({
                ...
                processData: false,
                beforeSend: function () {
                    $("#Footer_CreateProcesoExpediente_Enabled").css("display", "none");
                    $("#Footer_CreateProcesoExpediente_Disabled").css("display", "block");
                },
                success: function (response) {
                    var mensaje = response.mensaje;
                    form[0].reset();
                    $("#Footer_CreateProcesoExpediente_Enabled").css("display", "block");
                    $("#Footer_CreateProcesoExpediente_Disabled").css("display", "none");
                    $("#modalCreateProcesoExpediente").modal('hide');
                    $("#viewDataProcesoExpediente").html("<i class='fa fa-spinner fa-pulse fa-2x fa-fw'></i><span class='sr-only'>Cargando...</span><h5>Espere un momento por favor, obteniendo datos ...</h5>");
                    $("#viewDataProcesoExpediente").load(urlApp+'/proceso-pdn/proceso/'+ codExpediente +'/data');
                ...
                error: function (response) {
                    ...
                    $("#ProcesoExpedienteAlerts").css("display", "block");
                    $("#ProcesoExpedienteAlerts").html('<h4><i class="icon fa fa-exclamation-triangle"></i> Error: ' + errorTitle + '</h4>' + errorsHtml);
                    $("#Footer_CreateProcesoExpediente_Enabled").css("display", "block");
                    $("#Footer_CreateProcesoExpediente_Disabled").css("display", "none");
                }
            });
        });
        $(document).on("click", '#btnUpdateProcesoExpediente', function (event) {
            event.preventDefault();
            var form = $("#FormUpdateProcesoExpediente");
            var urlAction = form.attr('action');
            var formData = new FormData(form[0]);
            var dataAll = form.serialize();
            $.ajax({
                ...
                processData: false,
                beforeSend: function () {
                    $("#Footer_UpdateProcesoExpediente_Enabled").css("display", "none");
                    $("#Footer_UpdateProcesoExpediente_Disabled").css("display", "block");
                },
                success: function (response) {
                    var mensaje = response.mensaje;
                    form[0].reset();
                    $("#Footer_UpdateProcesoExpediente_Enabled").css("display", "block");
                    $("#Footer_UpdateProcesoExpediente_Disabled").css("display", "none");
                    $("#modalUpdateProcesoExpediente").modal('hide');
                    $("#viewDataProcesoExpediente").html("<i class='fa fa-spinner fa-pulse fa-2x fa-fw'></i><span class='sr-only'>Cargando...</span><h5>Espere un momento por favor, obteniendo datos ...</h5>");
                    $("#viewDataProcesoExpediente").load(urlApp+'/proceso-pdn/proceso/'+ codExpediente +'/data');
                    alertify.success(mensaje);
                },
