...
@section('title', 'Reporte consolidado de Convenios suscritos por el PCC')
@section('content')
                ...
                <div class="card-body">
                    <div class="ibox-content">
                        ...
                        <div class="form-group">
                            <div class="row">
                                <div class="col-md-12">
                                    <a href="#" id="btnConsultaListadoConvenio" class="btn btn-primary btn-sm"><i class="fas fa-search"></i> Realizar consulta</a>
                                </div>
                            </div>
                        </div>
                        <hr>
                        <div id="data_listado_convenio" class="table-responsive"></div>
                    </div>
                </div>
            </div>
        </div>
...
@section('scripts')
    <script>
        $(document).ready(function () {
            ...
            $("#data_listado_convenio").html("<i class='fa fa-spinner fa-pulse fa-2x fa-fw'></i><span class='sr-only'>Cargando...</span><h5>Espere un momento por favor, el reporte se esta procesando ...</h5>");
            $('#data_listado_convenio').load('convenio-consolidado/100/100/100/data');
            $("#boton").html("<a href='convenio-consolidado/100/100/100/excel' class='btn btn-warning btn-sm'><i class='fas fa-download'></i> Descargar Consolidado</a>");
            //2. Obtengo el resultado de la consulta
            $('#btnConsultaListadoConvenio').click(function () {
                var tipo        =   $("#inputTipo").val();
                var periodo     =   $("#inputPeriodo").val();
                var estado      =   $("#inputEstado").val();
                $("#data_listado_convenio").html("<i class='fa fa-spinner fa-pulse fa-2x fa-fw'></i><span class='sr-only'>Cargando...</span><h5>Espere un momento por favor, el reporte se esta procesando ...</h5>");
                $('#data_listado_convenio').load("convenio-consolidado/"+tipo+"/"+periodo+"/"+estado+"/data");
                $("#boton").html("<a href='convenio-consolidado/"+tipo+"/"+periodo+"/"+estado+"/excel' class='btn btn-warning btn-sm'><i class='fas fa-download'></i> Descargar Consolidado</a>");
            });
        });
    </script>
@stop