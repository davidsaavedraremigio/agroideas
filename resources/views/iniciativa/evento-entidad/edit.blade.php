{!!Form::model($oa,['id'=>'FormUpdateEntidad', 'method'=>'PATCH', 'files' => 'true', 'enctype' => 'multipart/form-data', 'route'=>['evento-entidad.update',$oa->id]])!!}
...
<div class="modal-body">
    ...
    <div class="form-group">
        <div class="row">
            <div class="col-md-4">{!! Form::label('ruc', 'Nro de RUC') !!} {!! Form::text('ruc', $entidad->nroDocumento, ['class' => 'form-control', 'placeholder' => '00000000000', 'maxlength' => '11', 'id' => 'input_nro_documento']) !!}</div>
    ...
    <div class="form-group">
        <div class="row">
            <div class="col-md-12 col-lg-12 col-xs-12">{!! Form::label('nombre', 'Razon social') !!}{!! Form::text('nombre', $entidad->nombre, ['class' => 'form-control', 'readonly' => 'readonly', 'id' => 'input_nombre']) !!}</div>
        </div>
    </div>
    <div class="form-group">
        <div class="row">
            <div class="col-md-4">{!! Form::label('ubigeo', 'Código ubigeo') !!} {!! Form::text('ubigeo', $entidad->ubigeo, ['class' => 'form-control', 'readonly' => 'readonly', 'id' => 'input_ubigeo']) !!}</div>
            <div class="col-md-8">{!! Form::label('direccion', 'Dirección') !!} {!! Form::text('direccion', $entidad->direccion, ['class' => 'form-control', 'readonly' => 'readonly', 'id' => 'input_direccion']) !!}</div>
        </div>
    </div>
    <div class="form-group">
        <div class="row">
            <div class="col-md-12">{!! Form::label('compromiso', 'Seleccione el compromiso al que corresponde esta Organización') !!}
                <select name="compromiso" class="form-control select2">
                    ...
                    @foreach ($compromisos as $fila)
                        <option value="{{$fila->id}}" {{($fila->id == $oa->compromisoID)?'selected':''}}>{{$fila->compromiso}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-4">{!! Form::label('tipo_incentivo', 'Tipo de incentivo') !!}
                <select name="tipo_incentivo" class="form-control select2">
                    ...
                    <option value="{{$fila->Orden}}" {{($fila->Orden == $oa->incentivoID)?'selected':''}}>{{$fila->Nombre}}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-4">{!! Form::label('cadena_inicial', 'Cadena productiva inicial') !!} 
                <select name="cadena_inicial" class="form-control">
                    ...
                    <option value="{{$fila->id}}" {{($fila->id == $oa->codCadenaInicial)?'selected':''}}>{{$fila->descripcion}}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-4">{!! Form::label('cadena_propuesta', 'Cadena productiva') !!}
                <select name="cadena_propuesta" class="form-control">
                    <option value="" selected="selected">Seleccionar</option>
                    @foreach ($cadenaProductiva as $fila)
                    <option value="{{$fila->id}}" {{($fila->id == $oa->codCadenaPropuesta)?'selected':''}}>{{$fila->descripcion}}</option>
                    @endforeach
                </select>
            </div>
        </div>
    ...
    <div class="form-group">
        <div class="row">
            <div class="col-md-4">{!! Form::label('nro_productores', 'Nro de productores (aprox.)') !!} {!! Form::text('nro_productores', $oa->nroProductores, ['class' => 'form-control']) !!}</div>
            <div class="col-md-4">{!! Form::label('nro_hectareas', 'Nro de Has (aprox.)') !!} {!! Form::text('nro_hectareas', $oa->nroHas, ['class' => 'form-control']) !!}</div>
            <div class="col-md-4">{!! Form::label('inversion', 'Inversión PCC (S/)') !!} {!! Form::text('inversion', $oa->inversion, ['class' => 'form-control']) !!}</div>
        </div>
    </div>
</div>
<div class="modal-footer justify-content-between">
    <div id="Footer_UpdateEntidad_Enabled">
        ...
        <a href="#" id="btnUpdateEntidad" class="btn btn-primary btn-sm"><i class="fas fa-save"></i> Guardar cambios</a>
    </div>
